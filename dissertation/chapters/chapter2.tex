...
More recently, a popular used activation function for \ac{DL} (see Section~\ref{sec:deep-learning}) is the \ac{ReLU} function~\autocite{ramachandran2017}, which is defined as the positive part of its input, \ie{}, \(\operatorname{ReLU}(x) = \max(0, x)\).
...
\begin{enumerate}
	...
	\item The target is fixed, and the prediction cannot be directly changed, only the weights.
		  ...
		  By iteratively applying the chain rule at each layer, the calculation can propagate backwards though the layers until the derivative at any layers.
